...
static int elf_check(const u8 *data, u64 size)
{
    ...
    if (eh->e_ident[4] != ELF_CLASS64)   { printf("[ELF] not ELF64\n");        return -1; }
    if (eh->e_ident[5] != ELF_DATA2LSB)  { printf("[ELF] not little-endian\n"); return -1; }
    if (eh->e_type != ELF_TYPE_EXEC &&
        eh->e_type != ELF_TYPE_DYN)   { printf("[ELF] not executable (e_type=%u)\n", (u32)eh->e_type); return -1; }
    if (eh->e_machine != ELF_MACH_X86_64){ printf("[ELF] not x86-64\n");       return -1; }
...
int elf_load(const u8 *data, u64 size, const char *name)
{
    ...
    printf("[ELF] loading '%s': type=%u machine=0x%x entry=0x%llx phdrs=%u\n",
           name, (u32)eh->e_type, (u32)eh->e_machine, eh->e_entry, (u32)eh->e_phnum);
    ...
    for (u16 i = 0; i < eh->e_phnum; i++)
    {
        ...
        printf("[ELF] seg %u: vaddr=0x%llx filesz=%llu memsz=%llu flags=%c%c%c\n",
               (u32)i, ph->p_vaddr, ph->p_filesz, ph->p_memsz,
               (ph->p_flags & PF_R) ? 'R' : '-',
               (ph->p_flags & PF_W) ? 'W' : '-',
               (ph->p_flags & PF_X) ? 'X' : '-');
        ...
        {
            printf("[ELF] seg %u out of bounds (off=%llu filesz=%llu total=%llu)\n",
                   (u32)i, ph->p_offset, ph->p_filesz, size);
            process_destroy(p);
            return -1;
        }
        ...
        if (ph->p_flags & PF_W)
            vmm_flags |= VMM_REGION_WRITE;
        if (ph->p_flags & PF_X)
            vmm_flags |= VMM_REGION_EXEC;

        u64 mapped = vmm_space_alloc(p->space, va_base, pg_count, vmm_flags);
        if (!mapped)
        {
            printf("[ELF] seg %u vmm_space_alloc failed (va=0x%llx pages=%llu)\n",
    ...
    if (!stack)
    {
        printf("[ELF] stack alloc failed (base=0x%llx pages=%llu)\n",
               USER_STACK_BASE, USER_STACK_PAGES);
        process_destroy(p);
        return -1;
    }

    printf("[ELF] stack mapped: 0x%llx - 0x%llx\n", USER_STACK_BASE, USER_STACK_TOP);
    ...
    printf("[ELF] launched '%s' entry=0x%llx stack_top=0x%llx\n",
           name, eh->e_entry, USER_STACK_TOP);
